...
use Cstopery\AlipayUserCertify\Libarys\Zmop\RSAUtil;
use Cstopery\AlipayUserCertify\Libarys\Zmop\WebUtil;
use Cstopery\AlipayUserCertify\Libarys\lotusphp_runtime\Logger\LtLogger;
class ZmopClient {
	//应用ID
	public $appId;
	//私钥文件路径，绝对路径或者相对路径
	...
	public function execute($request) {
		$bizContent = $this->getBizContent($request);
		$sysParams = $this->getSystemParams($request);
		$sysParams['biz_content'] = json_encode($bizContent,true);
		$sysParams[$this->signParam] = RSAUtil::sign($sysParams, $this->privateKeyFilePath);


		//加密,只对业务参数进行加密
//		$encryptedApiParams[$this->bizParam] = RSAUtil::rsaEncrypt($apiParamsQuery, $this->zhiMaPublicKeyFilePath);
//		$encryptedApiParams[$this->bizParam] = $apiParamsQuery;
		...
		//发起HTTP请求
		try {
			$resp = WebUtil::curl($requestUrl, null ,$this->charset);
		} catch (Exception $e) {
...
//            $this->logCommunicationError($sysParams["method"], $requestUrl, "HTTP_RESPONSE_NOT_WELL_FORMED", $respObject);

            $biz_response = $this->get_biz_response($respObject);
//			$encrypted = $respObject->encrypted;
//			if(isset($respObject->encrypted) && $respObject->encrypted){
//				$biz_response = RSAUtil::rsaDecrypt($biz_response, $this->privateKeyFilePath);
//			}
			$bizRespObject = $biz_response;
			if (null !== $respObject) {
				$respWellFormed = true;
			}
		}
...
//        if(isset($respObject['sign'])){
//            $bizResponseSign = $respObject['sign'];
...
//        $request->setIdentityParam(json_encode(array("identity_type"=>"CERT_INFO","cert_type"=>"IDENTITY_CARD","cert_name"=>$name,"cert_no"=>$idcard)));
    ...
    private function getBizContent($request){
